...
def test_validate_email_address_accepts_normal_address_and_rejects_bad_input():
    assert validate_email_address("recipient@example.com") == "recipient@example.com"
...
def test_build_completion_message_contains_counts_and_elapsed_time():
    ...
    message = build_completion_message("recipient@example.com", report)
...
def test_send_completion_email_logs_in_and_sends_message():
    ...
    config = EmailConfig(sender="sender@example.com", app_password="abcd efgh")
    ...
    assert calls[1] == ("login", "sender@example.com", "abcdefgh")
    assert calls[2] == ("send", "recipient@example.com", "sender@example.com")
...
def test_send_completion_email_rejects_missing_credentials():
    ...
    with pytest.raises(ValueError, match="sender"):
        send_completion_email(config, "recipient@example.com", report)
